...
export function Select(props) {
  ...
  const css = (/* css */`
    ...
    div {
      position: relative; /* Important for the positioning of the ul */
      ...
      ul {
        ...
        margin: 0;
        box-shadow: 0 0.1875em 0.125em -0.1em rgba(0, 0, 0, 0.2);
        border: 0.125em solid var(--color-background-400);
        overflow-y: auto;
  ...
  return template((x) => {
    ...
    const ul = x.ul(
      {
        ...
        onClick: (ev) => {
          ...
          if (typeof props.onChange === 'function') {
            ...
            if (option) {
              props.onChange(ev, option, option.value)
            }
          }
        }
      },
    ...
    const inputContainer = x.div(button, ul)
